...
function DisplayGradeAttend(config){
	...
	function aggregateData(data){
		...
		results.students.forEach(function(d){
				let key = d.key[0];
				// only get the names of students that are in the class
				if(!results.rates[key]) return;
	...
	function RenderTable(results){
		...
		Object.values(results.rates)
			...
			.map(function(d){
				let html = [
						" <td><span class='indicator alert-"+d.gradeAlert+"' title='"+d.grade.toFixed(1)+"'> </span></td>",
						" <td><span class='indicator alert-"+d.attendAlert+"' title='"+d.attend.toFixed(1)+"' > </span></td>",
